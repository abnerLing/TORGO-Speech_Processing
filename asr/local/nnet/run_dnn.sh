...
if [ $stage -le 2 ]; then
  ...
  steps/nnet/decode.sh --nj 1 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.1 \
    $gmm/graph $data_fmllr/test $dir/decode #cris graph
fi


echo "Re-train the DNN by 5 iterations of sMBR (State Minimum Bayes Risk, discriminative training)"
...
if [ $stage -le 4 ]; then
  # Re-train the DNN by 5 iterations of sMBR 
  ...
  # Decode
  for ITER in 5 3 1; do
    steps/nnet/decode.sh --nj 1 --cmd "$decode_cmd" --config conf/decode_dnn.config \
      --nnet $dir/${ITER}.nnet --acwt $acwt \
      $gmm/graph $data_fmllr/test $dir/decode_it${ITER} #cris graph
  done 
fi
